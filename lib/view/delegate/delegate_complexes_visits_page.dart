...
import '../../bindings/delegate_complexes_visits_binding.dart';
import '../../controller/delegate_complexes_visits_controller.dart';
import 'add_visit_page.dart';

class DelegateComplexesVisitsPage extends StatelessWidget {
  const DelegateComplexesVisitsPage({super.key});
  ...
  Widget build(BuildContext context) {
    if (!Get.isRegistered<DelegateComplexesVisitsController>()) {
      DelegateComplexesVisitsBinding().dependencies();
    }
    final controller = Get.find<DelegateComplexesVisitsController>();
  ...
  Widget _buildVisitsList(DelegateComplexesVisitsController controller) {
    ...
    final visits = controller.complexesVisits;

    if (visits.isEmpty) {
      return Center(
        child: MyText(
          'لا توجد زيارات للمجمعات الطبية',
          fontSize: 16.sp,
