...
class ChatController extends GetxController {
  ...
  Future<void> openDoctorChat(String doctorId, String doctorName) async {
    ...
    try {
      ...
      if (res['ok'] == true) {
        ...
        if (data is Map) {
          final convId = data['conversationId']?.toString() ?? data['_id']?.toString() ?? '';
      ...
      if (res['ok'] == true) {
        ...
        if (res['data'] != null && res['data']['data'] != null) {
          ...
          messages.add({
            ...
            'sender': 'me',
            'isMe': true,
            'createdAt': newMessage['createdAt']?.toString() ?? DateTime.now().toIso8601String(),
          });
        } else {
  ...
  void _extractReceiverFromMessages() {
    ...
    if (sender is Map && receiver is Map) {
      ...
      if (senderId == currentUserId) {
        ...
        print('Set receiver as: ID=$receiverIdFromMsg, Name=$receiverNameFromMsg');
      } else {
  ...
  Future<void> _enrichConversationsWithNames(List<Map<String, dynamic>> convList) async {
    final SessionController session = Get.find<SessionController>();
    final currentUserId = session.currentUser.value?.id ?? '';
        ...
        if (res['ok'] == true) {
          ...
          if (messages.isNotEmpty) {
            ...
            if (sender is Map && receiver is Map) {
              ...
              print('Conversation $convId: participant = ${conv['participantName']} (${conv['participantId']})');
            }
          }
        }
      } catch (e) {
